...
<body>
    <div class="login-container">
        ...
        <footer>
            ...
            <div class="version-info" id="version-info" style="display: none;">
                <span id="version-current" style="font-weight: 600;">Portal</span>
                <span class="version-badge file" id="version-file">
                    ...
                    <span>File: <span id="version-file-val">-</span></span>
                </span>
                <span class="version-badge db" id="version-db">
                    <svg class="version-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <ellipse cx="12" cy="5" rx="9" ry="3"/>
                        <path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/>
                        <path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/>
                    </svg>
                    <span>DB: <span id="version-db-val">-</span></span>
                </span>
                <span class="version-badge" id="version-status"></span>
            </div>
        </footer>
    </div>
    ...
    <script>
        ...
        async function fetchVersionInfo() {
            try {
                const response = await fetch(API_URL + '/version.php');
                const data = await response.json();
                ...
                if (data.success) {
                    ...
                    const fileEl = document.getElementById('version-file-val');
                    const dbEl = document.getElementById('version-db-val');
                    const statusEl = document.getElementById('version-status');

                    currentEl.textContent = 'Portal ' + info.current_version;
                    fileEl.textContent = info.file.version;
                    dbEl.textContent = info.database.version || 'N/A';

                    if (info.versions_match) {
                        statusEl.className = 'version-badge synced';
                        statusEl.innerHTML = '<svg class="version-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> Synced';
                    } else {
                        statusEl.className = 'version-badge mismatch';
                        statusEl.innerHTML = '<svg class="version-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> Mismatch';
                    }
