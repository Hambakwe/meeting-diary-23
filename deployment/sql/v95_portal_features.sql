...
--
-- Usage:
-- mysql -u username -p database_name < v95_portal_features.sql
-- ============================================================
...
-- Get event IDs and add attendees
...
-- ============================================================
-- 11. UPDATE VERSION TRACKING (optional)
-- ============================================================

-- Add portal_version to track which portal features are installed
-- This is safe as it only creates if not exists
...
('portal_version', 'v95', 'Client Portal version'),
('portal_features', 'documents,contacts,diary,dashboard,timeline', 'Enabled portal features'),
('portal_installed_at', NOW(), 'Portal features installation date'),
('portal_build_date', '2026-02-21', 'Portal build date')
ON DUPLICATE KEY UPDATE `value` = VALUES(`value`), `updated_at` = NOW();

-- Force update portal_version to v95 (in case ON DUPLICATE KEY didn't work)
UPDATE `system_settings` SET `value` = 'v95', `updated_at` = NOW() WHERE `key` = 'portal_version';
...
SELECT 'Portal v95 features installed successfully!' AS message;
